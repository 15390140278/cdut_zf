...
<template>
  <div>
    <el-breadcrumb separator="/">
      <el-breadcrumb-item>记录您拥有的房子的出租情况</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
      <el-table
        :data="
      ...
      >
        ...
        <el-table-column prop="title" label="房屋信息" width="120"></el-table-column>
        <el-table-column label="几室" width="50" prop="roomTypeId"></el-table-column>
        <el-table-column prop="areaId" label="地区" width="50"></el-table-column>
        <el-table-column prop="size" label="面积" width="50"></el-table-column>
        <el-table-column prop="floorId" label="楼层" width="50"></el-table-column>
        <el-table-column prop="rentTypeId" label="出租方式" width="80"></el-table-column>
        <el-table-column prop="priceNum" label="价格" width="80"></el-table-column>
        <el-table-column prop="userId" label="联系人" width="80"></el-table-column>
        <el-table-column prop="rentTypeId" label="朝向" width="80"></el-table-column>
        <el-table-column prop="updateTime" label="创建日期" width="80"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="300px">
          <template slot-scope="scope">
            <div class="btns">
              <el-button type="warning" size="mini" v-if="scope.row.state === 0" @click="showEditDialog(scope.row)">编辑</el-button>
              <el-button @click="houseInfo(scope.row)" type="primary" size="mini">详情</el-button>
              <el-button type="danger" size="mini" @click="deleteHouse(scope.row.id)">删除</el-button>
              <el-button round @click="saleHouse(scope.row)" type="info" size="mini" v-if="scope.row.state === 0">发布</el-button>
              <el-button round @click="cancelSale(scope.row)" type="info" size="mini" v-if="scope.row.state === 1">取消审核</el-button>
              <el-button round @click="cancelSale(scope.row)" type="info" size="mini" v-if="scope.row.state === 2">下架</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 查看房屋详情 -->
    <el-dialog title="房屋详情" :visible.sync="detailDialogVisible" width="50%">
      <div class="detail-span">
        设施：
        <el-tag class="detail-tag" v-for="item in deviceData" :key="item" type="primary" effect="plain">{{ item }}</el-tag>
      </div>

      <div class="detail-span">
        特点：
        <el-tag class="detail-tag" v-for="item in characteristicsData" :key="item" type="danger" effect="plain">{{ item }}</el-tag>
      </div>
    ...
    <!-- 编辑房屋 -->
    <el-dialog title="编辑房屋" :visible.sync="editDialogVisible" width="80%" @closed="editDialogClosed">
      <el-form :model="editForm" :rules="editFormrules" ref="editFormRef" label-width="100px" class="demo-ruleForm">
        <el-form-item label="房屋标题" prop="title">
          <el-input placeholder="请输入房屋标题" v-model="editForm.title"></el-input>
        </el-form-item>
        ...
        <el-form-item label="面积" prop="size">
          <el-input placeholder="请输入面积" v-model="editForm.size"></el-input>
        </el-form-item>
        <el-form-item label="房屋详情" prop="detail">
          <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="editForm.detail"></el-input>
        </el-form-item>
        <el-form-item label="出租方式" prop="rentTypeId">
          <el-select v-model="editForm.rentTypeId" placeholder="请选择">
            <el-option v-for="item in rentTypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="房屋朝向" prop="orientedId">
          <el-select v-model="editForm.orientedId" placeholder="请选择">
            <el-option v-for="item in orentedOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  created() {
    this.queryId.id = getItem('ZF_USER').id
    this.getHouseList()
  },
  data() {
    return {
  ...
  methods: {
    // 获取
    async getHouseList() {
      const { data: res } = await this.$http.get('house/selectHouseByUserId', {
        params: {
          id: this.queryId.id
        }
      })
    ...
    async deleteHouse(val) {
      console.log(val)
      const confirmResult = await this.$confirm('请问是否要删除房屋', '提示', {
      ...
      // 没有取消就是要删除，发送请求完成删除
      const { data: res } = await this.$http.post('house/delete', this.qs.stringify({ id: val }))
      console.log(res)
    ...
    // 发布
    async saleHouse(row) {
      const { data: res } = await this.$http.post('house/release', this.qs.stringify({ id: row.id }))
      if (res.code !== 200) {
        return this.$message.error('房屋发布失败')
      }
      this.$message.success('房屋发布成功')
      this.getHouseList()
    },
    // 下架、取消审核
    async cancelSale(row) {
      const { data: res } = await this.$http.post('house/cancelSale', this.qs.stringify({ id: row.id }))
      if (res.code !== 200) {
    ...
    async houseInfo(row) {
      ...
      const { data: res } = await this.$http.get('house/detail', { params: { id: row.id } })
      if (res.code !== 200) {
    ...
    async editHouse() {
      ...
      this.$refs.editFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.post('house/update?' + str)
        if (res.code !== 200) {
          console.log(res.msg)
